...
int main() {
  ...
  try {
    ...
    while (read_samples(samples) > 0) {
      ...
      // Initialise ASCII keyboard and populate it with a lambda expression.
      // I've given the capture variable an unusual name to highlight the fact
      // that it hasn't been declared beforehand. I've also used decltype
      // instead of the much simpler auto.
